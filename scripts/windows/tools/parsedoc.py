#!/usr/bin/env python3
# parsedoc <文件> — 提取文档文字/内容到 stdout（Windows 版）。
# PDF(pymupdf 正文 + pdfplumber 表格；无文字层→Qwen-VL OCR) / docx(python-docx) /
# rtf,odt(pandoc 顶替 mac 的 textutil) / xlsx(openpyxl) / 纯文本。CC 直接调，无需选库。
...
def parse_via_pandoc(path):  # rtf/odt：Windows 无 textutil，用 pandoc 转纯文本
    try:
...
if ext == "pdf":
    ...
    die("[parsedoc] Windows 不支持旧版 .doc，请先转成 .docx")
elif ext == "xlsx":
    parse_xlsx(F)
